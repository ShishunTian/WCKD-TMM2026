...
class HyperNet(nn.Module):
    ...
    """
    def __init__(self, lda_out_channels, hyper_in_channels, target_in_size, target_fc1_size, target_fc2_size, target_fc3_size,
                 target_fc4_size, feature_size, device, config):
        ...
        if self.config.cov or self.config.cross_correlation or self.config.fusion or self.config.cross_fusion:
            ...
            ckpt = torch.load('/data/hlz/pretrained_model/deeplabv3_resnet50_coco-cd0a2569.pth', map_location=torch.device('cuda:' + self.config.gpunum))
            deeplabv3.load_state_dict(ckpt, strict=False)
            deeplabv3 = deeplabv3.backbone
            for param in deeplabv3.parameters():
                param.requires_grad = False

            self.deeplabv3 = deeplabv3.to(device)

            self.iqa_features = []
            self.seg_features = []

        if self.config.fusion:
            if self.config.dataset in ['koniq-10k', 'kadid10k']:
                self.conv4096 = nn.Sequential(
                    nn.Conv2d(2048 * 2, 2048, 1),
                    nn.ReLU(inplace=True),
                    nn.Dropout(0.3)
                ).to(device)
            else:
                self.fusion_conv = nn.Sequential(
                    nn.Conv2d(2048 * 2, 2048, 3, padding=1, bias=False),
                    nn.BatchNorm2d(2048),
                    nn.ReLU(inplace=True),
                    nn.Conv2d(2048, 2048, 3, padding=1, bias=False),
                    nn.BatchNorm2d(2048),
                    nn.ReLU(inplace=True),
                    nn.Conv2d(2048, 2048, 1),  # 最后降维到目标通道数
                    nn.Dropout(0.3)
                )
    ...
    def forward(self, img):
        ...
        if self.config.cross_correlation or self.config.cov:
            self.iqa_features = res_out['res_feat']
            self.seg_features = self.deeplabv3(img)

        if self.config.fusion:
            if self.config.dataset in ['koniq-10k', 'kadid10k']:
                hyper_in_feat = self.conv4096(torch.concat((res_out['hyper_in_feat'], self.deeplabv3(img)['layer4']), dim=1))
                hyper_in_feat += res_out['hyper_in_feat']
            else:
                hyper_in_feat = self.fusion_conv(torch.concat((res_out['hyper_in_feat'], self.deeplabv3(img)['layer4']), dim=1))

        elif self.config.cross_fusion:
            iqa_feat = res_out['hyper_in_feat']
            seg_feat = self.deeplabv3(img)['layer4']
            ...
            hyper_in_feat = attn_features.permute(0, 2, 1).view(b, c, h, w)
        else:
            hyper_in_feat = res_out['hyper_in_feat']
    ...
    def get_features(self):
        return self.iqa_features, self.seg_features
...
class ResNetBackbone(nn.Module):
    ...
    def forward(self, x):
        ...
        out['res_feat'] = {'layer1': layer1_feat, 'layer2': layer2_feat, 'layer3': layer3_feat, 'layer4': layer4_feat}
...
def resnet50_backbone(lda_out_channels, in_chn, pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model_hyper pretrained on ImageNet
    """
