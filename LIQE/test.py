...
from liqe_utils import set_dataset, _preprocess2, set_session
...
for session in range(0, 10):
    ...
    model_name = 'LIQE_WCKD.pt' if config.fusion and config.cov else 'LIQE_Origin.pt'

    ckpt = os.path.join(config.sv_path, str(session+1), model_name)
    checkpoint = torch.load(ckpt, map_location= device)
    model.load_state_dict(checkpoint['model_state_dict'])
    session = set_session(session, ckpt)
    ...
    srcc1, plcc1 = compute_metrics(q_hat1, q_mos1)
    srcc2, plcc2 = compute_metrics(q_hat2, q_mos2)
    srcc3, plcc3 = compute_metrics(q_hat3, q_mos3)
    srcc4, plcc4 = compute_metrics(q_hat4, q_mos4)
    srcc5, plcc5 = compute_metrics(q_hat5, q_mos5)
    srcc6, plcc6 = compute_metrics(q_hat6, q_mos6)
...
def final_avg(all_srcc, all_plcc):
    median_srcc = np.mean(np.array(all_srcc))
    median_plcc = np.mean(np.array(all_plcc))

    return [round(median_srcc, 4), round(median_plcc,4)]


# live_results
live_results = final_avg(all_srcc['live'], all_plcc['live'])
csiq_results = final_avg(all_srcc['csiq'], all_plcc['csiq'])
bid_results = final_avg(all_srcc['bid'], all_plcc['bid'])
clive_results = final_avg(all_srcc['clive'], all_plcc['clive'])
koniq10k_results = final_avg(all_srcc['koniq10k'], all_plcc['koniq10k'])
kadid10k_results = final_avg(all_srcc['kadid10k'], all_plcc['kadid10k'])
