...
import random
import time
from MNL_Loss import loss_m4, Multi_Fidelity_Loss, Fidelity_Loss_distortion
import scipy.stats
from args import Configs
from liqe_utils import fuse_features, set_dataset, _preprocess2, _preprocess3, convert_models_to_fp32
...
# region textual template
dists = ['jpeg2000 compression', 'jpeg compression', 'white noise', 'gaussian blur', 'fastfading', 'fnoise', 'contrast',
         'lens', 'motion', 'diffusion', 'shifting',
...
type2label = {'jpeg2000 compression': 0, 'jpeg compression': 1, 'white noise': 2, 'gaussian blur': 3, 'fastfading': 4,
              'fnoise': 5, 'contrast': 6, 'lens': 7, 'motion': 8,
              'diffusion': 9, 'shifting': 10, 'color quantization': 11, 'oversaturation': 12, 'desaturation': 13,
              'white with color': 14, 'impulse': 15, 'multiplicative': 16, 'white noise with denoise': 17,
              'brighten': 18,
...
dist_map = {'jpeg2000 compression': 'jpeg2000 compression', 'jpeg compression': 'jpeg compression',
            ...
            'white noise with denoise': 'noise', 'brighten': 'overexposure', 'darken': 'underexposure',
            'shifting the mean': 'other',
            'jitter': 'spatial', 'noneccentricity patch': 'spatial', 'pixelate': 'spatial',
            'quantization': 'quantization',
            'color blocking': 'spatial', 'sharpness': 'contrast', 'realistic blur': 'blur', 'realistic noise': 'noise',
            'underexposure': 'underexposure', 'overexposure': 'overexposure', 'realistic contrast change': 'contrast',
...
scene2label = {'animal': 0, 'cityscape': 1, 'human': 2, 'indoor': 3, 'landscape': 4, 'night': 5, 'plant': 6,
               'still_life': 7,
               'others': 8}
# endregion
...
live_set = '/data/hlz/IQA_Database/databaserelease2/'
csiq_set = '/data/hlz/IQA_Database/CSIQ/'
bid_set = '/data/hlz/IQA_Database/BID/'
clive_set = '/data/hlz/IQA_Database/ChallengeDB_release/'
koniq10k_set = '/data/hlz/IQA_Database/koniq-10k/'
kadid10k_set = '/data/hlz/IQA_Database/kadid-10k/'
...
def do_batch(x, text):
    ...
    dino_features = dinov2.get_intermediate_layers(x, layer_list)

    if config.fusion:
        fuse_features(liqe_features, dino_features, layer_list, device)

    if config.cov:
        loss_cov = 0.0
        for i in range(0, len(layer_list)):
            cov_matrix_layer = CovMatrix(dim=768, clusters=3, device=device)
            eps = 1e-5
            IQA_feature = liqe_features[i].permute(1, 2, 0)
            ...
            loss_isw = cov_matrix_layer.instance_whitening_loss(dist_cor, diff_mat)
            loss_cov += loss_isw
        return logits_per_image, loss_cov
    else:
...
def train(model, best_result, best_epoch, srcc_dict, scene_dict, type_dict):
    ...
    for step in range(num_steps_per_epoch):
        ...
        optimizer.zero_grad()
        if config.cov:
            logits_per_image, loss_cov = do_batch(all_batch, joint_texts)
            all_loss = [loss_cov]
            total_loss = loss_cov
        else:
        ...
        if mtl == 0:
            ...
            all_loss.extend(calc_loss(logits_quality, gmos_batch.detach(), num_sample_per_task,
            ...
            all_loss.extend(calc_loss2(logits_quality, gmos_batch.detach(), num_sample_per_task,
                                       logits_scene, scene_gt_batch.detach())
                            )
            ...
            all_loss.extend(calc_loss3(logits_quality, gmos_batch.detach(), num_sample_per_task,
                                  logits_distortion, dist_gt_batch.detach())
                            )
    ...
    if (epoch >= 0):
        ...
        quality_result['test'] = {'live': srcc11, 'csiq': srcc22, 'bid': srcc33, 'clive': srcc44, 'koniq10k': srcc55,
                                  'kadid10k': srcc66}
        scene_result['val'] = {'live': scene_acc1, 'csiq': scene_acc2, 'bid': scene_acc3, 'clive': scene_acc4,
                               'koniq10k': scene_acc5,
                               'kadid10k': scene_acc6}
        scene_result['test'] = {'live': scene_acc11, 'csiq': scene_acc22, 'bid': scene_acc33, 'clive': scene_acc44,
                                'koniq10k': scene_acc55,
                                'kadid10k': scene_acc66}
        distortion_result['val'] = {'live': dist_acc1, 'csiq': dist_acc2, 'bid': dist_acc3, 'clive': dist_acc4,
                                    'koniq10k': dist_acc5,
                                    'kadid10k': dist_acc6}
        distortion_result['test'] = {'live': dist_acc11, 'csiq': dist_acc22, 'bid': dist_acc33, 'clive': dist_acc44,
                                     'koniq10k': dist_acc55,
                                     'kadid10k': dist_acc6}
        ...
        if current_avg > best_result['avg']:
            ...
            ckpt_name = os.path.join(sv_path, 'avg_best_ckpt.pt')
            torch.save({
                'epoch': epoch,
        ...
        if srcc_avg > best_result['quality']:
            ...
            ckpt_name = os.path.join(sv_path, 'quality_best_ckpt.pt')
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
        ...
        if scene_avg > best_result['scene']:
            ...
            ckpt_name = os.path.join(sv_path, 'scene_best_ckpt.pt')
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
        ...
        if dist_avg > best_result['distortion']:
            ...
            ckpt_name = os.path.join(sv_path, 'distortion_best_ckpt.pt')
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
...
for session in range(0, 10):
    print(f'This is Session ' + str(session + 1))

    sv_path = os.path.join(config.sv_path, str(session + 1))
    ...
    dinov2 = torch.hub.load('./dinov2', 'dino_vitb16', source='local').to(device)
    ...
    def hook_fn(module, input, output):
        liqe_features.append(output)

    for i in layer_list:
        layer_of_interest = model.visual.transformer.resblocks[i]
    ...
    live_train_loader = set_dataset(live_train_csv, 4, live_set, 8, preprocess3, train_patch, False)
    ...
    csiq_train_loader = set_dataset(csiq_train_csv, 4, csiq_set, 8, preprocess3, train_patch, False)
    ...
    bid_train_loader = set_dataset(bid_train_csv, 4, bid_set, 8, preprocess3, train_patch, False)
    ...
    clive_train_loader = set_dataset(clive_train_csv, 4, clive_set, 8, preprocess3, train_patch, False)
    ...
    koniq10k_train_loader = set_dataset(koniq10k_train_csv, 16, koniq10k_set, 8, preprocess3, train_patch, False)
    ...
    kadid10k_train_loader = set_dataset(kadid10k_train_csv, 16, kadid10k_set, 8, preprocess3, train_patch, False)
    ...
    train_loaders = [live_train_loader, csiq_train_loader, bid_train_loader, clive_train_loader, koniq10k_train_loader,
                     kadid10k_train_loader]

    result_pkl = {}
    for epoch in range(0, num_epoch):
        best_result, best_epoch, srcc_dict, scene_dict, type_dict, all_result = train(model, best_result, best_epoch,
                                                                                      srcc_dict,
    ...
    pkl_name = os.path.join(sv_path, 'all_results.pkl')
    with open(pkl_name, 'wb') as f:
        pickle.dump(result_pkl, f)

    lambdas = weighting_method.method.lambda_weight
    pkl_name = os.path.join(sv_path, 'lambdas.pkl')
    with open(pkl_name, 'wb') as f:
