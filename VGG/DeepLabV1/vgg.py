...
def extract_res(inp, model):
    deeplab_features = []
    for index, module in model.features.named_children():  # 按执行顺序遍历网络各层操作
        inp = module(inp)  # 逐层前向执行，得到结果
        if index in ["3", "8", "15", "22", "29"]:
            deeplab_features.append(inp)
    return deeplab_features


if __name__ == "__main__":
    device = 'cuda:1'
    model = VGG16_LargeFOV().to(device)
    model.load_state_dict(torch.load('deeplabv1.pth', map_location=device))
    x = torch.ones([2, 3, 321, 321]).to(device)
    deeplab_features = extract_res(x, model)
    print(deeplab_features)